...
void CSkeletonBasics::DrawSkeleton( NUI_SKELETON_DATA & skel, int windowWidth, int windowHeight)
{      


	for (int j = 0; j < NUI_SKELETON_POSITION_COUNT; ++j) 
	{
		skel.SkeletonPositions[j].x = CONST_FLOAT;
		skel.SkeletonPositions[j].y = CONST_FLOAT;
		skel.SkeletonPositions[j].z = CONST_FLOAT;
	}
    int i;
    ...
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; ++i)
    {
		...
		float point = m_Points[0].x;
		wchar_t WR_char = wchar_t(point);

		SendDlgItemMessageW(m_hWnd, IDC_STATUS, WM_SETTEXT, 0, 20);
